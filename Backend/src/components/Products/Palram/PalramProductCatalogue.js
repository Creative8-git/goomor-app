...
const PalramProductCatalogue = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/palramproducts/${productId}`);
        if (!response.ok) {
  ...
  if (!product) return <div>Product not found. Please check the product ID in the URL.</div>;

  const hasImageSamples = product.colors?.some((color) => color.url);

  return (
    <div className="container mt-5 Palram">
      <div className="row">
...
export default PalramProductCatalogue;
