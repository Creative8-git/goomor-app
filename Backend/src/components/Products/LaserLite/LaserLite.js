...
const LaserLite = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/laserliteproducts`);
        if (!response.ok) {
  ...
  const items = products.map((product) => ({
    name: product.title,
    link: `/laserlite/${product.id}`,
    image: product.carouselImages?.[0] || "/images/placeholder.png", // Fallback for missing images
  }));

  return (
    <div className="container-fluid service bg-light pb-5 LaserLite">
      <div className="container pb-5">
        <div className="text-center mx-auto pb-5 wow fadeInDown" data-wow-delay="0.1s" style={{ maxWidth: "800px" }}>
          <img
            src="/images/LaserLite-logo.svg"
            alt="LaserLiteLogo"
