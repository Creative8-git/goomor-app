...
const StratCo = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/stratcoproducts`); // Replace with your API endpoint
        if (!response.ok) throw new Error("Failed to fetch StratCo products");
...
const handleGoToPage = () => {
  ...
  const categories = [
    { name: "StratCo Roofing", items: roofingProducts },
    { name: "StratCo Wall Cladding", items: wallingProducts },
    { name: "StratCo Rainwater Products", items: rainwaterProducts },
  ];

  return (
    <div className="container-fluid service bg-light pb-5 StratCo">
      <div className="container pb-5">
        <div className="text-center mx-auto pb-5 wow fadeInUp" data-wow-delay="0.2s" style={{ maxWidth: "800px" }}>
          <img
            src="images/stratco-logo.svg"
            alt="StratCo Logo"
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="service-content text-center h-100 justify-content-center p-3">
                        <Link to={`/stratco/${item.id}`} className="d-block h-100 text-dark fs-5 my-2 text-decoration-none text-white">
                          {item.title}
                        </Link>
                      </div>
                    </div>
