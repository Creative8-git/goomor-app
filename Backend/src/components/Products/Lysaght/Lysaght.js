...
const Lysaght = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_API_URL}/api/lysaghtproducts`);
                if (!response.ok) throw new Error("Failed to fetch Lysaght products");
    ...
    const totalPages = Math.max(1, Math.ceil(totalItems / itemsPerPage)-3);
    ...
    const categories = [
        { name: "Lysaght Roofing", items: roofingProducts },
        { name: "Lysaght Wall Cladding", items: wallingProducts },
        { name: "Lysaght Rainwater Products", items: rainwaterProducts },
    ];

    return (
        <div className="container-fluid service bg-light pb-5 Lysaght">
            <div className="container pb-5">
                <div className="text-center mx-auto pb-5 wow fadeInUp" data-wow-delay="0.2s" style={{ maxWidth: "800px" }}>
                    <img
                        src="images/lysaght-logo.svg"
                        alt="Lysaght Logo"
                                    ...
                                    >
                                        ...
                                        >
                                            <div className="service-img" style={{ height: "200px", overflow: "hidden" }}>
                                                <img
                                                    src={item.carouselImages?.[0] || "images/placeholder.png"} // Placeholder for missing images
                                                    className="img-fluid w-100 h-100 object-fit-cover"
                                                    alt={item.title}
                                                />
                                            </div>
                                            <div className="service-content text-center h-100 justify-content-center p-3">
                                                <Link to={`/lysaght/${item.id}`} className="d-block h-100 text-dark fs-5 my-2 text-decoration-none text-white">
                                                    {item.title}
                                                </Link>
                                            </div>
                                        </div>
