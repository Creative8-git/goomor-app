...
const LysaghtProductCatalogue = () => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_API_URL}/api/lysaghtproducts/${productId}`);
                if (!response.ok) {
    ...
    return (
        <div className="container mt-5 Lysaght">
            <div className="row">
                <div className="col-md-6 mb-4">
                    <CustomCarousel images={product.carouselImages} />
                    <p>Recommended Flashing</p>
...
export default LysaghtProductCatalogue;
