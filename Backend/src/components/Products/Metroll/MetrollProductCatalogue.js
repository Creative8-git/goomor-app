...
const MetrollProductCatalogue = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/metrollproducts/${productId}`);
        if (!response.ok) {
  ...
  return (
    <div className="container mt-5 Metroll">
      <div className="row">
...
export default MetrollProductCatalogue;
