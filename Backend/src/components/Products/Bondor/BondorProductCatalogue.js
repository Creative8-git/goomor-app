...
const BondorProductCatalogue = () => {
    ...
    const fetchProduct = async () => {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/api/bondorproducts/${productId}`
        );
        if (!response.ok) {
  ...
  return (
    <div className="container mt-5 Bondor">
      <div className="row">
...
export default BondorProductCatalogue;
