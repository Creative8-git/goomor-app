...
const Bondor = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/bondorproducts`);
        if (!response.ok) {
  ...
  const items = products.map((product) => ({
    name: product.title,
    link: `/bondor/${product.id}`,
    image: getAssetUrl(product.carouselImages?.[0]) || "/images/placeholder.png", // Use placeholder for missing images
  }));

  return (
    <div className="container-fluid service bg-light pb-5 Bondor">
      <div className="container pb-5">
        <div className="text-center mx-auto pb-5 wow fadeInDown" data-wow-delay="0.1s" style={{ maxWidth: "800px" }}>
          <img
            src="/images/bondor-logo.png" // Replace with the actual path to your logo
            alt="BondorLogo"
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      ...
                      alignItems: "center",
                      height: "50px", // Uniform content section height
                    }}
                  >
